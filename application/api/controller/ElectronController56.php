...
//            }
    ...
    //获取新的认证接口token
    public function xintoken(){
        $url="http://api.yexiaowu.top:28720/invoices/getToken";
        $date=['userId'=>'user875309821','password'=>'875309821YP'];
        $json=json_encode($date);
        $res=$this->http_post_json($url,$json);
    ...
    public function billcheck(){
        ...
        $url="http://api.yexiaowu.top:28720/invoices/invoiceVerify";
        $token=$this->xintoken();
        if($token==''){
            return $this->json_error('查询失败，可使用手动查询或拍照识别！');
        }
        $list=$this->tocurl($url,$token,$content);
        ...
        if($re['code']==0){
            ...
            if($qq['type']=='01'){
                ...
                $arr=$this->jsfpx($qq,$sqje);
            }elseif($qq['type']=='14'){
                $arr=$this->abcx($qq,$ree['date']);
            }elseif($qq['type']=='15'){
                $arr=$this->escfpx($qq);
            }else{
            ...
            $arr['ctime']=time();
            $arr['whole']=json_encode($qq);//返回的完整的值
            $id = $billinfo->getAdddata($arr);
            if ($id) {
                $where = ['id' => $id];
                $field = 'id,uid,typeid,photourl,invoicetype,photodate,opendate,consumetype,billcode,invoicecode,
   ...
   /* //祥云官网接口（验真）
    ...
    public function jdcx($qq){
        ...
        $arr = [
            ...
            'salebank' => $qq['khyh_jdcfp'],//销售方开户行及账号
