...
use mylib\oss;
class EduceController extends BaseController
{
	...
	public function addexport(){
		...
		$field='id,invoicetype,photodate,opendate,consumetype,billcode,invoicecode,pretax,taxamount,
			...
			name,idcard,frompl,topl,mileage,seattype';
		if ($data['type']==1) {
		    if($data['folderid']=='-1'){
                $where=['uid'=>$res['id'],'state'=>0,'checkbill'=>1,'soutype'=>0];
            }else{
                $where=['uid'=>$res['id'],'state'=>0,'folderid'=>$data['folderid']];
            }
	...
	public function ec($data,$title,$mu){
        $e = myphpexcel::exportfolder($data,$title,$mu);
        return $e;
    }
